...
export const home = async (req, res) => {
    try {
      ...
      const recentChallenges = await Challenge.find({})
        .sort({ createdAt: -1 }) // 생성일을 기준으로 내림차순 정렬
        .limit(5)
        .exec();
  
      // 홈 페이지를 렌더링합니다.
      return res.render('home', {
        pageTitle: '헬스인', // 페이지 제목을 '헬스인'으로 설정
        top5Contents: top5Posts,
        recentChallenges: recentChallenges // 최근 챌린지 데이터 추가
      });
    } catch (error) {
      console.error('Error fetching top posts and recent challenges:', error);
      // 오류 발생 시 홈 페이지를 렌더링합니다.
      return res.render('home', {
        pageTitle: '헬스인', // 페이지 제목을 '헬스인'으로 설정
        top5Contents: [], // 오류 발생 시 빈 배열로 설정
        recentChallenges: [] // 오류 발생 시 빈 배열로 설정
      });
    }
  };
